...
import { addFlowerCategory, addBadge } from '@/store/catalogMetaSlice';
...
export default function FlowersList() {
  ...
  const [draft, setDraft] = useState<FlowerItem | null>(null);
  const [saving, setSaving] = useState(false);

  async function load() {
    setLoading(true);
    const data = await dataProvider.listFlowers();
    setItems(data);
    setLoading(false);
  }
  useEffect(() => {
  ...
  const categories = useAppSelector((s) => s.catalogMeta.flowerCategories);
  ...
  function openEdit(item: FlowerItem) {
    setDraft({ ...item });
    setDialogOpen(true);
  }
  async function persist() {
    if (!draft) return;
    setSaving(true);
    if (items.find((i) => i.id === draft.id)) await dataProvider.updateFlower(draft);
    else
  ...
  async function confirmDelete() {
    if (deleteId) {
      await dataProvider.deleteFlower(deleteId);
      await load();
      setDeleteId(null);
    }
  }
    ...
    <div className="space-y-6">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {draft?.id && items.find((i) => i.id === draft.id) ? 'Edit Flower' : 'New Flower'}
            </DialogTitle>
            <DialogDescription>Manage catalog entry details.</DialogDescription>
          </DialogHeader>
          {draft && (
                                    ...
                                    !categories.some(
                                      ...
                                      <CommandItem
                                        ...
                                        onSelect={() => {
                                          ...
                                          if (newCat) {
                                            dispatch(addFlowerCategory(newCat));
